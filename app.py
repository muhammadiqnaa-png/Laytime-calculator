...
def build_pdf(ctx):
    ...
    styles.add(ParagraphStyle(name="SubHeader", fontSize=10, spaceBefore=4, spaceAfter=4, textColor=colors.darkblue))
    elems = []
    ...
    t_info.setStyle(TableStyle([("GRID", (0,0), (-1,-1), 0.25, colors.grey),
                                ...
                                ("FONTSIZE", (0,0), (-1,-1), 9)]))
    elems += [Spacer(1,1), Paragraph("<b>Information</b>", styles["SubHeader"]), t_info, Spacer(1,1)]
    ...
    t_sum.setStyle(TableStyle([("GRID", (0,0), (-1,-1), 0.25, colors.grey),
                               ...
                               ("FONTNAME", (0,0), (-1,-1), "Helvetica")]))
    elems += [Paragraph("<b>Summary</b>", styles["SubHeader"]), t_sum, Spacer(1,1)]

            # 🔧 Tambahkan footer di bawah dokumen
    today = datetime.now().strftime("%d %B %Y")
    footer_text = f"Generated by https://laytime-calculator-byiqna.streamlit.app/ — {today}"
    ...
    t_ttd = Table(
        [
            [
                Paragraph(f"<b>{pt_input}</b>", ttd_style_center),
                Paragraph(f"<b>{pt_shipper}</b>", ttd_style_center),
            ],
            [
                Paragraph("<br/><br/><br/><br/><br/>--------------------------", ttd_style_center),
                Paragraph("<br/><br/><br/><br/><br/>-----------------------------", ttd_style_center),
            ],
        ],
    ...
    # Bungkus biar center pas tengah halaman
    t_ttd_outer = Table([[t_ttd]], colWidths=[520], hAlign="CENTER")
    elems.append(t_ttd_outer)
...
company_name = st.text_input("Company Name", "PT. Pelayaran Armada Nasional Indonesia")
